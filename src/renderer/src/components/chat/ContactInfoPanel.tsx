  ...
  return (
    <div className="w-72 flex-shrink-0 border-l border-gray-200 bg-white flex flex-col overflow-y-auto">
      ...
      <div className="flex flex-col items-center px-5 pb-4">
        <div className="relative mb-3">
          {isDirect && avatarUserId ? (
            <Avatar userId={avatarUserId} name={name} avatarVersion={partner?.avatarVersion} className="w-24 h-24 rounded-full flex-shrink-0" />
          ) : (
          ...
          {isDirect && status && (
            <span className={`absolute right-1.5 bottom-1.5 w-4 h-4 rounded-full border-2 border-white ${STATUS_DOT[status] ?? 'bg-gray-400'}`} />
          )}
        </div>
        <div className="text-gray-900 text-lg font-medium text-center truncate max-w-full">{name}</div>
        {isDirect && status && (
          <div className="text-gray-500 text-sm">{STATUS_LABEL[status] ?? status}</div>
        )}
      </div>

      <div className="px-4 py-3 border-t border-gray-100 flex flex-col gap-1">
        <button type="button" onClick={() => partner && setShowProfile(true)} disabled={!isDirect} className="flex items-center gap-3 px-2 py-2.5 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      <div className="px-6 py-4 border-t border-gray-100 flex flex-col gap-4">
        {isDirect && status && (
          <div>
            <div className="text-[11px] font-medium text-gray-400 uppercase tracking-wide mb-1.5">Status</div>
        ...
        {isDirect && accountType && (
          <div>
            <div className="text-[11px] font-medium text-gray-400 uppercase tracking-wide mb-1.5">Tipe akun</div>
            <div className="text-sm text-gray-700">{accountType}</div>
          </div>
        )}
        {isDirect && partner?.jobTitle && (
          <div>
            <div className="text-[11px] font-medium text-gray-400 uppercase tracking-wide mb-1.5">Jabatan</div>
            <div className="text-sm text-gray-700">{partner.jobTitle}</div>
            {partner.jobDepartment && <div className="text-xs text-gray-400">{partner.jobDepartment}</div>}
          </div>
        )}
      </div>

      {showProfile && partner && (
        <PartnerProfileDialog partner={partner} onClose={() => setShowProfile(false)} />
      )}
    </div>
  )
}
